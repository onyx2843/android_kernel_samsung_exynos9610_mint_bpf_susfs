...
static void susfs_run_sus_path_loop_internal(void) {
...
static void susfs_run_extra_works(struct work_struct *work) {
	if (!ksu_cred)
		return;
	#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	susfs_run_sus_path_loop_internal();
	#endif // #ifdef CONFIG_KSU_SUSFS_SUS_PATH
}

/* public wrapper for KernelSU-Next compat (uid param unused in v2.2.0) */
void susfs_run_sus_path_loop(uid_t uid) {
	susfs_run_sus_path_loop_internal();
}

/* deprecated stubs for KernelSU-Next compat */
void susfs_set_i_state_on_external_dir(void __user **user_info) {
	// deprecated in v2.2.0, no-op
}
...
void susfs_try_umount(uid_t uid) {
	// no-op for now, handled by KernelSU-Next's own ksu_handle_umount
}
